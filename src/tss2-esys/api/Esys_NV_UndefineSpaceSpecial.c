    ...
    ESYS_TR shandle3)
{
    ...
    iesys_compute_session_value(esysContext->session_tab[0],
                                nvIndexNode == NULL ? NULL : &nvIndexNode->rsrc.name,
                                nvIndexNode == NULL ? NULL : &nvIndexNode->auth);
    iesys_compute_session_value(esysContext->session_tab[1],
                                platformNode == NULL ? NULL : &platformNode->rsrc.name,
                                platformNode == NULL ? NULL : &platformNode->auth);
    ...
    ESYS_CONTEXT *esysContext)
{
    ...
    if (nvIndexNode == NULL || session == NULL) {
        return TSS2_ESYS_RC_BAD_REFERENCE;
    }
